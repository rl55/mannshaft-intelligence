...
class OrchestratorAgent:
    ...
    def __init__(
        ...
        self.evaluator = evaluator or Evaluator(self.cache_manager)
        # Initialize HITL manager after guardrail_agent to avoid circular import
    ...
    async def _execute_analytical_agents(
        ...
        """
        # Map orchestrator analysis_type to agent-specific analysis_type
        # Agents expect 'comprehensive' for full analysis, or specific types
        # For orchestrator types like 'comprehensive', 'quick', 'deep_dive', 
        # we use 'comprehensive' for individual agents
        ...
        spreadsheet_ids = {
            ...
            'support': config.get('sheets.support.spreadsheet_id')
        }
        ...
        # Create tasks for parallel execution with agent-specific contexts
        tasks = []
    ...
    async def _handle_hitl_escalations(
        ...
        risk_score = self._calculate_escalation_risk_score(
            violations, response, evaluation_details
        )
        ...
        # Escalate if needed
        if critical_violations or risk_score > 0.7:
            try:
                # Extract trace_id from response if available
                trace_id = None
                if isinstance(response, dict):
                    try:
                        response_data = json.loads(response.get('response', '{}')) if isinstance(response.get('response'), str) else response
                        trace_id = response_data.get('metadata', {}).get('trace_id')
                    except (json.JSONDecodeError, TypeError):
                        pass
                
                decision = await self.hitl_manager.escalate(
                    session_id=session_id,
                    report=response,
                ...
                self.logger.info(
                    f"HITL escalation completed: {decision.decision}",
                    extra={
                ...
                self.logger.error(
                    f"Failed to escalate to HITL: {e}",
                    exc_info=True
                )
        
        return hitl_count
    
    def _calculate_escalation_risk_score(
        self,
        violations: List[Dict[str, Any]],
        response: Dict[str, Any],
        evaluation_details: Optional[Dict[str, Any]]
    ) -> float:
        """Calculate risk score for escalation decision."""
        risk_score = 0.0
        ...
        if violations:
            ...
            risk_score += critical_count * 0.4
            risk_score += high_count * 0.2
        
        # Risk from evaluation
        if evaluation_details:
            overall_score = evaluation_details.get('evaluation_details', {}).get('overall_score', 1.0)
            if overall_score < 0.7:
                risk_score += (0.7 - overall_score) * 0.5
    ...
    def _determine_escalation_reason(
        ...
        if any(v.get('rule_type') == 'hard' for v in violations):
            reasons.append("Hard guardrail violation")
        
        if any(v.get('severity') == 'critical' for v in violations):
            reasons.append("Critical severity violation")
        ...
        if isinstance(response, dict):
            try:
                ...
                confidence = response_data.get('confidence', 1.0)
                if confidence < 0.7:
                    reasons.append("Low confidence with business-critical insights")
            except (json.JSONDecodeError, TypeError):
                pass
        ...
        if isinstance(response, dict):
            try:
                response_data = json.loads(response.get('response', '{}')) if isinstance(response.get('response'), str) else response
                contradictions = response_data.get('analysis', {}).get('correlations', [])
                if len(contradictions) > 0:
                    reasons.append("Contradictory signals from agents")
            except (json.JSONDecodeError, TypeError):
                pass
        ...
        if not reasons:
            reasons.append("High-risk scenario requiring human review")
    ...
    async def _evaluate_quality(
        ...
        Args:
            response: Synthesized response
            session_id: Session identifier
            analytical_results: Optional original analytical results for context
        ...
        try:
            ...
            # Prepare original data summary for grounding check
            original_data_summary = None
            if analytical_results:
                original_data_summary = {
                    'revenue_metrics': analytical_results.get('revenue', {}).get('metadata', {}),
                    'product_metrics': analytical_results.get('product', {}).get('metadata', {}),
                    'support_metrics': analytical_results.get('support', {}).get('metadata', {})
                }
            ...
            # Evaluation is already recorded in evaluator.evaluate()
